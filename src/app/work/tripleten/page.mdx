...
TripleTen’s approach is built around project-based learning—and projects are the biggest, most complex part of it. Most of a student’s time is spent working on projects, and that’s also where they need the most help.
...
It was a big UX challenge. I had to rethink the whole platform layout so students could work comfortably in a complex environment while still having quick access to all their usual tools. The goal was to make it feel like the same familiar place—just upgraded into a full professional workspace.

Another major challenge was simplifying how projects work. Their workflows could get really complex, so I had to break them down into the clearest possible model. Now each project consists of just a few steps that make immediate sense to students, while their layout and look can still vary depending on the project type and status.

<div className="wide-block">![Different states](/tripleten/pop/states.png)</div>

It was a fun design project but also a big technical one—making the platform and external workspace work smoothly together. With a tight-knit team of engineers, we built the first VS Code version in three months, but the full rollout, iteration, and support for more tools took another year.
